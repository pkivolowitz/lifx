...
# ---------------------------------------------------------------------------
# Server-install CLI shim — /usr/local/bin/glowup
# ---------------------------------------------------------------------------
# Mirrors the standalone shim's role: a brand-new server-install user
# can run ``glowup discover`` from any shell instead of memorising the
# ``/opt/glowup/venv/bin/python /opt/glowup/repo/glowup.py …`` long
# form.  The shim also auto-exports ``GLOWUP_TOKEN`` from
# ``/etc/glowup/server.json`` (when readable) so server-routed commands
# (``--device``, group CRUD, identify) work without the user separately
# fishing the token out of the JSON.  /etc/glowup is mode 0750 owned by
# ``glowup`` group, so a user invoking the shim needs to be in that
# group; if they aren't, the token export is a no-op and the user can
# still run direct-UDP commands (``--local discover``, ``--ip play``).

_SERVER_SHIM_PATH: Path = Path("/usr/local/bin/glowup")
"""System-wide CLI launcher dropped by ``run_server``."""

_SERVER_SHIM_TEMPLATE: str = """#!/usr/bin/env bash
# GlowUp launcher (server install) — auto-generated by install.py.
# Invokes the system venv's Python on the deployed glowup.py.
# Auto-loads the server's auth token from /etc/glowup/server.json
# when the invoking user can read it (members of the 'glowup' group).
# To regenerate: re-run sudo ./install.sh.
if [ -z "${{GLOWUP_TOKEN:-}}" ] && [ -r /etc/glowup/server.json ]; then
    GLOWUP_TOKEN="$({venv_python} -c 'import json;print(json.load(open("/etc/glowup/server.json")).get("auth_token",""))' 2>/dev/null)"
    [ -n "$GLOWUP_TOKEN" ] && export GLOWUP_TOKEN
fi
exec {venv_python} {entry_point} "$@"
"""


def write_server_shim(host: HostInfo) -> None:
    """Render ``/usr/local/bin/glowup`` and chmod root:root 755.

    Always called as root (run_server has already escalated).  Idempotent
    — overwrites any existing shim so a re-run picks up venv / repo path
    changes.  The shim itself is a tiny bash wrapper; the heavy lifting
    is the auth-token export logic, which a brand-new operator would
    otherwise have to discover and configure by hand.
    """
    target = _SERVER_SHIM_PATH
    venv_py = OPT_DIR / "venv" / "bin" / "python3"
    entry = OPT_DIR / "repo" / "glowup.py"
    if not entry.is_file():
        # Should be unreachable: sync_runtime_repo runs first.  Guard
        # explicitly so a partial install fails loudly here rather than
        # leaving a broken /usr/local/bin/glowup.
        fail(f"glowup.py entry point not found at {entry}", exit_code=22)

    body = _SERVER_SHIM_TEMPLATE.format(
        venv_python=str(venv_py),
        entry_point=str(entry),
    )
    target.write_text(body, encoding="utf-8")
    # root:root owns it; world-readable + executable so any user on the
    # box can run ``glowup discover``.  The auth-token export inside the
    # shim only succeeds for users who can also read /etc/glowup/server.json
    # (mode 0640 root:glowup), which gates server-routed commands behind
    # group membership while leaving direct-UDP commands available to
    # everyone.
    target.chmod(0o755)
    ok(f"wrote launcher {target} → {entry}")
