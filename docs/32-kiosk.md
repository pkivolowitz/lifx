...
|----------|----------|---------|-------------|
| `server_ip` | Yes | -- | GlowUp server IP (e.g., `10.0.0.48`) |
...
```bash
# Basic setup.
sudo bash setup_clock.sh 10.0.0.48

# Bedroom clock: rotated display, BLE sensor daemon, custom hostname.
sudo bash setup_clock.sh 10.0.0.48 --hostname bedroom --rotate 90 --ble

# Remote setup via SSH.
ssh a@10.0.0.148 "bash -s" < tools/setup_clock.sh 10.0.0.48 --ble
```
